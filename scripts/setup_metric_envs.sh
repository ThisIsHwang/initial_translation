...
VLLM_ENV_FILE="${VLLM_ENV_FILE:-$ENV_ROOT/vllm_env.env}"
...
# Use vLLM env for align by default, keep generate on base env.
ALIGN_DEFAULT=""
if [ -d "$ENV_ROOT/vllm" ]; then
  ALIGN_DEFAULT="$ENV_ROOT/vllm"
fi

cat > "$ENV_PIPELINE_FILE" <<EOF
# Auto-generated by scripts/setup_metric_envs.sh
: "\${UV_PROJECT_GEN:=$ENV_GEN}"
: "\${UV_PROJECT_ALIGN:=${ENV_ALIGN:-$ALIGN_DEFAULT}}"
